...
the sliders. (The default values of the sliders might be suitable for a
recreational game of toss.)
...
coefficient to match an application of interest, and try revisiting some of your
analysis.
...
ui <- fluidPage(titlePanel(title="", windowTitle="River Transit"),
    ...
    fluidRow(
        column(3, sliderInput("m", value=0.150, min=0.01, max=0.25, step=0.01,
                label="Mass [kg]")),
        column(3, sliderInput("U", value=10.0, min=2.0, max=46.0, step=0.05,
                label="Speed [m/s]")),
        column(3, sliderInput("angle", value=20.0, min=0, max=90.0, step=0.1,
                label="Angle [deg]")),
        column(3, sliderInput("lambda", value=0.001, min=0, max=0.5, step=0.0005,
                label="Friction Coef. [kg/m]")),
        ),
...
server <- function(input, output, session)
{
    output$plot <- renderPlot({
        ...
        plot(x, z, type="l", lwd=2,
            ylim=c(0, 1.04*max(z)), xaxs="i", yaxs="i", xlab="Horizontal coordinate, x [m]", ylab="Vertical coordinate, z [m]")
        abline(h=0, col="tan")
        grid()
        if (landed)
            mtext(sprintf("Landed at x=%.2fm after %.1fs", xstar, tstar))
